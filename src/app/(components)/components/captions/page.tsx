...
const PresetItem = ({
    ...
    >
        <div
            className={`relative rounded-lg overflow-hidden bg-muted aspect-video w-full border transition-all duration-200 ${isActive ? "ring-2 ring-primary border-primary" : "border-border group-hover:border-primary/50"
                }`}
        >
            {video ? (
            ...
            {isActive && <div className="absolute inset-0 bg-primary/10 flex items-center justify-center"><div className="w-2 h-2 rounded-full bg-primary" /></div>}
        </div>
...
const CaptionsPage = () => {
        ...
        const applyCaptions = async () => {
            if (studioRef.current) {
                ...
                const styleUpdate: any = {
                    ...
                    fontUrl: preset.fontUrl || "https://fonts.gstatic.com/s/bangers/v13/FeVQS0BTqb0h60ACL5la2bxii28.ttf",
                    align: (preset.textAlign || "center") as any,
                    caption: { colors: { appeared: preset.appearedColor, active: preset.activeColor, activeFill: preset.activeFillColor, background: preset.backgroundColor, keyword: preset.isKeywordColor ?? "transparent" }, preserveKeywordColor: preset.preservedColorKeyWord ?? false },
                    animation: preset.animation || "undefined",
                    textCase: preset.textTransform || "normal",
                    dropShadow: { color: preset.boxShadow?.color ?? "transparent", alpha: 0.5, blur: preset.boxShadow?.blur ?? 4, distance: Math.sqrt(x * x + y * y) ?? 4, angle: Math.PI / 4 },
                ...
                for (const clip of allCaptionClips) {
                    await regenerateCaptionClips({ studio: studioRef.current, captionClip: clip, mode, fontSize: (clip as any).originalOpts?.fontSize, fontFamily: styleUpdate.fontFamily, fontUrl: styleUpdate.fontUrl, styleUpdate });
                }
    ...
    const handleSave = async (data: any) => {
        if (!session) { toast.error("Please sign in to save presets"); router.push("/signin"); return; }
        setIsSaving(true);
        try {
            const res = await fetch("/api/custom-presets", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: data.label || "My Caption", category: "captions", data }) });
        ...
        <div className="flex h-[calc(100vh-57px)] overflow-hidden">
            <div className="w-160 shrink-0 border-r flex flex-col overflow-hidden">
                <Tabs defaultValue="presets" className="flex flex-col flex-1 overflow-hidden">
                        ...
                        <ScrollArea className="h-full">
                            ...
                            >
                                {STYLE_CAPTION_PRESETS.map((preset, index) => {
                                    ...
                                    return <PresetItem key={index} label={label} video={preset.previewUrl} isActive={selectedPreset === label} onClick={() => handlePresetSelect(preset, label)} />;
                                })}
                            </div>
                        </ScrollArea>
                    </TabsContent>
                    ...
                    <TabsContent value="customize" className="flex-1 overflow-hidden mt-0">
                        <ScrollArea className="h-full">
                            <div className="p-4">
                                <CustomCaptionForm onApply={(data) => handlePresetSelect(data, "Custom")} onSave={handleSave} isSaving={isSaving} />
                            </div>
                        </ScrollArea>
                    </TabsContent>
                    ...
                    <TabsContent value="generate" className="flex-1 overflow-hidden mt-0">
                        <div className="h-full flex flex-col items-center justify-center gap-4 p-6 text-center">
                            <div className="p-4 rounded-2xl bg-primary/10 border border-primary/20"><Sparkles className="w-6 h-6 text-primary" /></div>
                            <div>
                                <Badge variant="secondary" className="mb-3">Coming Soon</Badge>
                                <h3 className="font-semibold text-sm">AI Caption Generator</h3>
                                <p className="text-xs text-muted-foreground mt-1.5 leading-relaxed max-w-50">Describe a caption style and let AI generate a custom preset for you.</p>
                            </div>
                        </div>
                    </TabsContent>
                </Tabs>
            ...
            <div className="flex-1 flex flex-col items-center justify-center bg-muted/30 p-6 overflow-hidden">
                <div className="w-full max-w-md">
                    <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-3 text-center">Preview</p>
                    <div className="relative rounded-xl overflow-hidden bg-black shadow-xl">
                        <ExamplePlayer project={project} onLoad={() => setLoading(false)} onReady={(s) => { studioRef.current = s; }} />
                        {loading && (
                    ...
                    {selectedPreset && <p className="text-center text-xs text-muted-foreground mt-3">Previewing: <span className="text-foreground font-medium">{selectedPreset}</span></p>}
                </div>
            </div>
        </div>
    );
