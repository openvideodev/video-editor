...
const AnimationsPage = () => {
    ...
    const handlePresetSelect = (preset: any) => {
        ...
        if (clip) clip.animations = [{ type: preset.value, opts: { duration: 1000000, delay: 0, easing: "easeOutQuad", iterCount: 1 } }];
    ...
    const handleCustomApply = (data: any) => {
        ...
        const newProject = JSON.parse(JSON.stringify(project));
        const clip = newProject.clips.find((c: any) => c.type === "Image");
        if (clip) { clip.animations = [{ type: data.type, opts: data.opts, params: data.params }]; toast.success("Animation applied"); }
        setProject(newProject);
        setLoading(true);
    };
    ...
    const handleSave = async (data: any) => {
        ...
        setIsSaving(true);
        try {
            const res = await fetch("/api/custom-presets", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: data.label || "My Animation", category: "animations", data }) });
        ...
        <div className="flex h-[calc(100vh-57px)] overflow-hidden">
            <div className="w-160 shrink-0 border-r flex flex-col overflow-hidden">
                ...
                <Tabs defaultValue="presets" className="flex flex-col flex-1 overflow-hidden">
                        ...
                        <ScrollArea className="h-full">
                            ...
                            >
                                {ANIMATION_PRESETS.map((preset) => (
                                    <PresetItem key={preset.value} label={preset.label} image={preset.previewStatic} isActive={selectedPreset === preset.value} onClick={() => handlePresetSelect(preset)} />
                                ))}
                            </div>
                        </ScrollArea>
                    </TabsContent>
                    ...
                    <TabsContent value="customize" className="flex-1 overflow-hidden mt-0">
                        <ScrollArea className="h-full">
                            <div className="p-4">
                                <CustomAnimationForm onApply={handleCustomApply} onSave={handleSave} isSaving={isSaving} />
                            </div>
                        </ScrollArea>
                    </TabsContent>
                    ...
                    <TabsContent value="generate" className="flex-1 overflow-hidden mt-0">
                        <div className="h-full flex flex-col items-center justify-center gap-4 p-6 text-center">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground mt-1.5 leading-relaxed max-w-50">Describe a motion and let AI create a custom animation preset for you.</p>
                            </div>
                        </div>
                    </TabsContent>
                </Tabs>
            ...
            <div className="flex-1 flex flex-col items-center justify-center bg-muted/30 p-6 overflow-hidden">
                <div className="w-full max-w-md">
                    ...
                    {selectedPreset && <p className="text-center text-xs text-muted-foreground mt-3">Previewing: <span className="text-foreground font-medium">{ANIMATION_PRESETS.find(p => p.value === selectedPreset)?.label}</span></p>}
                </div>
            </div>
        </div>
    );
