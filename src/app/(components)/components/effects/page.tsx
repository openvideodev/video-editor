...
const EffectsPage = () => {
    ...
    const handleCustomApply = (data: any) => {
        ...
        const newProject = JSON.parse(JSON.stringify(project));
        const clip = newProject.clips.find((c: any) => c.type === "Effect");
        if (clip) { clip.effect = { ...clip.effect, key: "custom", name: data.label, fragment: data.fragment }; toast.success("Custom effect applied"); }
        setProject(newProject);
        setLoading(true);
    };
    ...
    const handleSave = async (data: any) => {
        ...
        setIsSaving(true);
        try {
            const res = await fetch("/api/custom-presets", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: data.label || "My Effect", category: "effects", data }) });
        ...
        <div className="flex h-[calc(100vh-57px)] overflow-hidden">
                ...
                <Tabs defaultValue="presets" className="flex flex-col flex-1 overflow-hidden">
                    ...
                    <TabsContent value="presets" className="flex-1 overflow-hidden mt-0">
                        <ScrollArea className="h-full">
                            ...
                            >
                                {effects.map((e) => (
                                    <PresetItem key={e.key} label={e.label} image={e.previewStatic} isActive={selectedPreset === e.key} onClick={() => handlePresetSelect(e)} />
                                ))}
                            </div>
                        </ScrollArea>
                    </TabsContent>
                    ...
                    <TabsContent value="customize" className="flex-1 overflow-hidden mt-0">
                        <ScrollArea className="h-full">
                            <div className="p-4">
                                <CustomShaderForm type="Effect" onApply={handleCustomApply} onSave={handleSave} isSaving={isSaving} />
                            </div>
                        </ScrollArea>
                    </TabsContent>
                    ...
                    <TabsContent value="generate" className="flex-1 overflow-hidden mt-0">
                        <div className="h-full flex flex-col items-center justify-center gap-4 p-6 text-center">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground mt-1.5 leading-relaxed max-w-50">Describe a visual effect and let AI write a custom GLSL shader for you.</p>
                            </div>
                        </div>
                    </TabsContent>
                </Tabs>
            ...
            <div className="flex-1 flex flex-col items-center justify-center bg-muted/30 p-6 overflow-hidden">
                <div className="w-full max-w-md">
                    ...
                    {selectedPreset && <p className="text-center text-xs text-muted-foreground mt-3">Previewing: <span className="text-foreground font-medium">{effects.find(e => e.key === selectedPreset)?.label}</span></p>}
                </div>
            </div>
        </div>
    );
