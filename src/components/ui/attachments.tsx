...
function inferDetailedSubtitleMode(attachment: AttachmentMeta): "size" | "kind" {
  if (attachment.size != null && Number.isFinite(attachment.size) && attachment.size > 0) {
    return "size";
  }
  return "kind";
}
...
type AttachmentVariant = NonNullable<VariantProps<typeof attachmentVariants>["variant"]>;
...
function Attachments({
  ...
  const appendFilesFromList = React.useCallback(
    (rawFiles: File[], appendOptions?: AppendFilesOptions) => {
      ...
      const tooLarge = maxSize != null ? incoming.filter((f) => f.size > maxSize) : [];

      const withinSize = maxSize != null ? incoming.filter((f) => f.size <= maxSize) : incoming;

      const room =
        maxFiles != null ? Math.max(0, maxFiles - attachments.length) : Number.POSITIVE_INFINITY;

      const take = room === Number.POSITIVE_INFINITY ? withinSize : withinSize.slice(0, room);

      const overMaxFiles = room === Number.POSITIVE_INFINITY ? [] : withinSize.slice(room);
  ...
  React.useEffect(() => {
    ...
    const hasFiles = (e: DragEvent) =>
      Boolean(e.dataTransfer?.types?.length && [...e.dataTransfer.types].includes("Files"));
...
export type AttachmentsDropOverlayProps = Omit<React.HTMLAttributes<HTMLDivElement>, "children"> & {
  /**
...
export function AttachmentsDropOverlay({
  ...
  const { isDraggingFile, disabled, maxSize } = useAttachmentsContext("AttachmentsDropOverlay");
    ...
    >
      {children ?? (
        <div className="flex flex-col items-center gap-3">
          <RiUploadLine className="size-5 text-primary" />

          <div className="flex flex-col items-center gap-1">
            <p className="text-sm font-[350] text-primary">Drop files here to add as attachment</p>
            {maxSizeLabel ? (
...
function AttachmentTrigger({
  ...
  const { openPicker, disabled: rootDisabled } = useAttachmentsContext("AttachmentTrigger");
  ...
  const triggerClassName = cn(disabled && "cursor-not-allowed opacity-50", className);
...
type AttachmentProps = Omit<React.HTMLAttributes<HTMLDivElement>, "children"> & {
  variant?: AttachmentVariant;
...
function Attachment({
  ...
  const showProgress = !readOnly && progress != null && Number.isFinite(progress);
      ...
      <>
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          <AttachmentInfo>
            ...
            <AttachmentProperty as={detailedSubtitle === "size" ? "size" : "kind"} />
          </AttachmentInfo>
        </div>
      </>
    ) : (
  ...
  return (
    <AttachmentItemContext.Provider value={ctxValue}>
      ...
      >
        ...
        {showProgress && variant !== "pasted" ? <AttachmentProgress value={progress} /> : null}
      </div>
...
const attachmentPreviewVariants = cva(
  "flex shrink-0 items-center justify-center overflow-hidden bg-card text-muted-foreground",
  {
    variants: {
      variant: {
        compact: "absolute inset-0 size-full rounded-[inherit] border-0 bg-transparent",
...
type AttachmentPreviewProps = Omit<React.HTMLAttributes<HTMLDivElement>, "children"> &
...
function AttachmentPreview({
  ...
  const rasterSrc =
    ...
    (attachment.type === "image" && attachment.url ? attachment.url : undefined);
  const videoSrc =
  ...
  const content = (() => {
    ...
    const IconComponent = iconForAttachmentType(attachment.type);
    return <IconComponent className={iconClass} aria-hidden />;
  })();
...
function AttachmentRemove({
  ...
  const { variant, attachment, onRemove, readOnly } = useAttachmentItemContext("AttachmentRemove");
  const position = positionProp ?? (variant === "inline" ? "center-end" : "corner");

  const ariaLabel = ariaLabelProp ?? `Remove ${attachment.name ?? "attachment"}`;
  ...
  return (
    <button
      ...
      {...props}
    >
      {children ?? <RiCloseLine className="size-3" aria-hidden />}
    </button>
  );
}
...
type AttachmentPropertyProps = Omit<React.HTMLAttributes<HTMLParagraphElement>, "children"> & {
  as: AttachmentPropertyAs;
};

function AttachmentProperty({ as: mode, className, ...props }: AttachmentPropertyProps) {
...
function AttachmentProgress({ className, value, ...props }: AttachmentProgressProps) {
