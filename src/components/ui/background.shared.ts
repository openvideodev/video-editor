import { ASCII_CHARACTER_PALETTES, type AsciiCharacterPalette } from "./ascii-image";
...
export function normalizeFieldValue(value: number, minimum: number, maximum: number) {
  if (!Number.isFinite(value)) {
    return 0;
  }
...
export function computeBackgroundGridDimensions({
  ...
  const normalizedResolution = Number.isFinite(resolution) && resolution > 0 ? resolution : 0.1;
...
export function createAuroraEmitters(seed: number) {
  ...
  return Array.from(
    ...
    (): BackgroundEmitter => ({
      ...
      orbitRadiusX: 0.5 + random() * 0.22,
      orbitRadiusY: 0.5 + random() * 0.2,
      phase: random() * TAU,
...
export function writePointerTrailBlob(pool: PointerTrailPool, blob: PointerTrailBlob) {
